...
async function runTest(id) {
  ...
  expect(view.dom.innerHTML)
    .toBe('<div>index.properties</div>')
  expect(window.getComputedStyle(comp.querySelector('.inner').dom).color)
    .toBe('red')
  expect(window.getComputedStyle(comp.querySelector('.index').dom).color)
    .toBe('green')
  ...
  expect(view.dom.innerHTML)
    .toBe('<div>comp3.properties</div>')

  expect(comp.instance.print())
    .toBe(123)
}
